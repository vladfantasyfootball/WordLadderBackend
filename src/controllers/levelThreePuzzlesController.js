...
export const getPreviousLevelThreePuzzle = async () => {
    const now = new Date();
    const today = new Date(now.getTime() - 7 * 60 * 60 * 1000);
    const releaseDate = new Date(constReleaseDate);
    const todayUTC = Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate());
    ...
    try {
        return await LevelThreePuzzlesModel.findOne({ id: diffDays - 1 });
    } catch (error) {
        console.error('Error getting previous level three puzzle:', error);
        return null;
    }
}
